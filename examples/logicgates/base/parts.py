...
# --------------------------------------------------------------------------- #
# Rewards  (cue[B], action[B], cue_next[B], p) -> reward[B]
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Games -- trial-based: return a per-lane cue sequence [B, n_steps]. Each step
# is one independent trial; the action does not change the cue stream.
# --------------------------------------------------------------------------- #
@register("game", "randstate")
def randstate(rngs, p, n_steps):
    """Random-state task: each trial shows a uniform random state in [0,n_states)."""
    B, n = p["_B"], p["n_states"]
...
def cue(rngs, p, n_steps):
    """Instrumental task: each trial presents a uniform random cue in [0,n_cues)."""
    B, n = p["_B"], p["n_cues"]
    return np.stack([rngs[b].integers(0, n, size=n_steps) for b in range(B)])
